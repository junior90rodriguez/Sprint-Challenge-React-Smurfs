...
- [ ] Explain the differences between `client-side routing` and `server-side routing`
  ...
  - Server-side Routing is when we request information from a server. 
  - Client-side Routing is when Routing is handled imternally by the JavaScript that is already on the page.
